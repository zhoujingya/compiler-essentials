#ifndef LLVM_TOOLS_MY_EXEGESIS_OPERANDGRAPH_H
#define LLVM_TOOLS_MY_EXEGESIS_OPERANDGRAPH_H
...
struct Node : public std::pair<NodeType, int> {
  ...
  void dump(const MCRegisterInfo &RegInfo) const;
};
...
private:
  // We use std::set to keep the implementation simple, using a unordered_set
...
// both ways.
void setupRegisterAliasing(const MCRegisterInfo &RegInfo, Graph &TheGraph);
